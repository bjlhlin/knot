...
int kqueue_ctx_remove(kqueue_ctx_t *set, unsigned i)
{
	...
	int update_size = 0;
	EV_SET(&update[update_size++], set->ev[i].ident, EVFILT_READ, EV_DELETE, 0, 0, (void*)(intptr_t)i);
	if (i < last) {
		set->ev[i] = set->ev[last];
		set->timeout[i] = set->timeout[last];
		set->usrctx[i] = set->usrctx[last];
		EV_SET(&update[update_size++], set->ev[last].ident, EVFILT_READ, EV_ADD|EV_ENABLE, 0, 0, (void*)(intptr_t)i);
	}
